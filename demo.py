...
os.environ['TORCH_HOME']='E:/huggingface'
...
ldm_stable = StableDiffusionPipeline.from_pretrained("E:\huggingface\stable-diffusion-v1-5",local_files_only = True).to(device)
...
if __name__ == "__main__":
    ...
    image_hide = ode.latent2image(latent_noise)
    cv2.imwrite("{:s}/hidenoise.png".format(args.save_path), cv2.cvtColor(image_hide, cv2.COLOR_RGB2BGR))
    ...
    image_hide = ode.latent2image(latent_noise)
    cv2.imwrite("{:s}/recovernoise.png".format(args.save_path), cv2.cvtColor(image_hide, cv2.COLOR_RGB2BGR))
